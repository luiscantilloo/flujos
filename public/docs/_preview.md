...
Comprar La cuenta genera una orden de compra al proveedor con líneas de productos y kg. La OC avanza por estados hasta que el proveedor envía el camión hacia la bodega.


---

Entrar a la bodega El custodio recibe físicamente la mercancía y cierra la recepción contra la OC: línea por línea qué kg llegaron, si hubo diferencias o líneas adicionales no pedidas.


---

Registrar ingreso El custodio da de alta las cajas (manual o asistido desde la OC): van a la zona de entrada con temperatura, peso, cliente comercial y vínculos al catálogo. La mercancía 'existe' en el sistema pero aún no ocupa el mapa principal.


---

Ubicar en el mapa El jefe (o custodio) crea órdenes de trabajo 'de entrada hacia bodega'. El operario las toma de la cola, ejecuta y la caja pasa del arreglo de entrada a un casillero numerado del mapa.


---

Permanencia y control En mapa, cada casillero muestra temperatura. Si supera el umbral o una orden lleva demasiado tiempo, nacen alertas; el jefe puede asignarlas y cerrarlas con motivo.


---

Transformar (opcional) Si la cuenta creó una solicitud de procesamiento, el jefe asigna un operario o procesador. Al ponerse en curso se descuenta kg del primario en el mapa; al cerrar se declaran merma y/o sobrante.


---

Vender La cuenta crea una orden de venta a un comprador. Cuando decide despachar, la venta puede pasar a transporte hacia una bodega destino o flujo interno.


---

Salida física El jefe/custodio genera órdenes de trabajo hacia la zona de salida; el operario ejecuta. Desde salida se despacha hacia camión.


---

Entrega al comprador Se abre un viaje numerado TV-####. El transportista registra cantidades entregadas, conformidad, foto/firma o incidencia, y al cerrar la venta queda Cerrado(ok) o Cerrado(no ok).
...
README.md — Portada del proyecto Alta ✅ Completo


---

Diagrama de arquitectura del sistema Alta ✅ Completo


---

Documentación de API (OpenAPI / Swagger) Alta ✅ Completo


---

Variables de entorno y configuración Alta ✅ Completo


---

Guía de instalación y ejecución local Alta ✅ Completo


---

CONTRIBUTING.md — Cómo contribuir Alta ✅ Completo


---

Glosario de términos del negocio Alta ✅ Completo


---

Flujos de negocio end-to-end Alta ✅ Completo


---

Architecture Decision Records (ADRs) Media ☐ Pendiente


---

Documentación de testing Media ✅ Completo


---

Runbooks de operación y deployment Media ✅ Completo


---

Guía de onboarding para nuevos devs Media ✅ Completo


---

CHANGELOG.md — Historial de versiones Media ✅ Completo


---

Documentación de seguridad y autenticación Media ✅ Completo


---

Definición de entornos (dev/staging/prod) Media ✅ Completo


---

Guía de observabilidad y monitoreo Baja ☐ Pendiente
...
Notas de migración entre versiones mayores Baja ✅ Completo
...
> **Archivo referenciado.** Archivo referenciado: README.md
...
dev, build, start, lint, test, test:all.

Estructura del proyecto Árbol de carpetas con la responsabilidad de cada directorio. Roles del sistema Tabla de los 9 roles con su función principal.

Rutas de la aplicación Tabla de rutas con su descripción.

Documentación relacionada Links a CONTRIBUTING.md, CHANGELOG.md, architecture.mermaid.

Troubleshooting rápido Los 4 problemas más comunes y sus soluciones.
...
> **Archivo referenciado.** Archivo referenciado: architecture.mermaid
...
Endpoints server-side — rutas POST de Next.js El sistema tiene dos rutas API server-side. Estas rutas requieren secretos que no pueden exponerse al cliente (API Secret de Cloudinary, URL del webhook de n8n), por eso viven en el servidor.
...
> **Nota.** Completar todas las variables según la sección 4 de este documento.
...
> **Archivo referenciado.** Archivo referenciado: CONTRIBUTING.md
...
OperadorCuentas Da de alta el producto 'Pechuga congelada'. Crea OC-0042 por 2.000 kg a Frigorífico Sur y marca fecha de llegada. La OC pasa a estado Transporte.


---

Custodio Recibe 1.980 kg (faltaron 20 kg). Cierra recepción con diferencias. La OC queda Cerrado(ok) o Cerrado(no ok) según política del negocio.


---

Custodio Registra cuatro cajas en la zona de entrada, vinculadas a la OC.


---

Jefe Crea cuatro órdenes de trabajo 'a bodega' para los casilleros 3, 4, 7 y 8.


---

Operario Toma las órdenes de la cola y las ejecuta. Cada caja pasa de entrada a su casillero.


---

Sistema Inventario en vivo refleja 1.980 kg repartidos. Historial registra ingresos y movimientos. Reportes muestran el stock.
...
OperadorCuentas Crea venta VE-0015 por 500 unidades contra catálogo existente.


---

Custodio Realiza el cartonaje contra la OV: registra físicamente las cajas a despachar, línea por línea.


---

Jefe/Sistema La venta pasa a Transporte. Se genera viaje TV-0048 con líneas esperadas y camión de la flota asignado.


---

Transporte Abre el viaje, verifica las líneas. Carga foto del pallet, marca 500 unidades conformes (o menos + incidencia), firma en pantalla.


---

Sistema Cierra el viaje. La OV pasa a Cerrado(ok) o Cerrado(no ok) según conformidad. El stock en mapa se reduce.
...
Cliente Crea solicitud de procesamiento por 500 kg de 'Carne en bloque' (primario) hacia 'Lonchas empaquetadas' (secundario), en bodega B-01.


---

Jefe Asigna la solicitud al procesador Juan.


---

Procesador (Juan) Pasa la solicitud a En curso. El sistema resta 500 kg de los casilleros donde había ese producto.


---

Procesador (Juan) Declara: merma 15 kg (pérdida real) y sobrante 8 kg de bloque que vuelve al producto primario. Pasa a Terminado.


---

Sistema Registra mermaProcesamientoKgTotal +15 en historial. Genera órdenes de trabajo: una para ubicar lonchas en casillero libre, otra para reintegrar los 8 kg de bloque.


---

Operario Ejecuta las órdenes generadas. Reportes de procesamiento cuadran kg.
...
> **Archivo referenciado.** Archivo referenciado: CHANGELOG.md
...
*Cómo migrar cuando hay cambios que rompen compatibilidad*

Para cada versión MAJOR, este documento debe incluir una sección de migración. A continuación, se establece la plantilla y los escenarios conocidos.
