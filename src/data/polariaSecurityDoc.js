...
export function formatPolariaSecurityMarkdown() {
  ...
  const lines = [
    ...
    '| `SensitiveWriteGuard` | Escrituras sensibles inventario/OT/procesamiento |',
    '',
    'Escrituras sensibles (bodegas, SOL/OC, inventario, OT, procesamiento) pasan por API con Prisma + `DATABASE_URL` (bypass RLS) y validación de tenant en código.',
    '',
    '### Permisos atómicos (frontend + API)',
    '',
    '| Permiso | Roles con acceso |',
    '| --- | --- |',
    '| `inventory:write` | configurador, administrador_bodega, jefe_bodega, operario |',
    '| `inventory:read` | Todos excepto procesador/transportista sin lectura |',
    '| `warehouse_state:write` | configurador, administrador_bodega, jefe_bodega, operario |',
    '| `counters:write` | configurador, administrador_bodega, jefe_bodega |',
    '',
    '### Mateo Support (Widget-react)',
    '',
    '| Flujo | Token | Duración |',
    '| --- | --- | --- |',
    '| Chat n8n | JWT widget (`POST /auth/mateo/widget-token`) | ~300s |',
    '| Historial conversaciones | Bearer sesión WMS | Sesión activa |',
    '| SSO WMS ↔ Mateo | Código handoff | 60s |',
    '',
    'Tablas: `widget_conversacion`, `widget_mensaje` (migraciones 051–052). RLS POL-138: `codigo_cuenta` coherente con tenant del caller.',
    '',
    '### Manejo de secretos',
    '',
    '| Secreto | Regla |',
